...
*Auto=Yes
    * Attributes and methods
    ...
    EXPORTED:
    METHOD New(cIDDatabase,cFileIni,lIsConfigEncrypted) 
...
* SOURCE
*/
METHOD New(cIDDatabase,cFileIni,lIsConfigEncrypted) CLASS ISQL
...
*Auto=Yes
    /* Default values */
    hb_Default( @cFileIni , "database.ini" )
...
*Auto=No
    ...
    ELSE
        IF ! File( cFileIni )
            LOG ::setLastMessage( _t( "The configuration file (filename: %s) not found", cFileIni ) ) ;
    ...
    IF lIsConfigEncrypted
        hIni := VLJ_DecryptINI( cFileIni )
    ELSE
        hIni := hb_iniRead( cFileIni )
    ENDIF       
    IF ! cIDDataBase $ hIni
        LOG ;
        ::setLastMessage( _t( "Database identify (id: %s) not found. Config file name: %s", cIDDataBase, cFileIni ) );
